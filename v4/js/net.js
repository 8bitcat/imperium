// IMPERIUM — multiplayer over PeerJS (host-authoritative)
// Samma beprövade mönster som de andra familjespelen (lantliv m.fl.).
const PREFIX = 'imperium-';
...
export class Net {
  constructor() {
    this.mode = null;       // 'host' | 'client' | null (solo)
    this.peer = null;
    this.conns = new Map(); // peerId -> conn (host side)
  ...
  broadcast(t, d, except) {
    for (const [id, c] of this.conns) if (id !== except && c.open) c.send({ t, d });
  }
  // host -> one client
  sendTo(peerId, t, d) { const c = this.conns.get(peerId); if (c?.open) c.send({ t, d }); }

  get playerCount() { return (this.mode === 'host' ? this.conns.size + 1 : this.conns.size); }

  destroy() {
    try { this.peer?.destroy(); } catch {}
    this.peer = null; this.conns.clear(); this.hostConn = null; this.mode = null;
  }
}
