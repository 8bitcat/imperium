// IMPERIUM — svenska landsnamn.
// world-atlas använder ISO 3166-1 numeriska koder som id. Vi mappar numerisk -> alpha-2
// och låter webbläsarens Intl.DisplayNames ge det svenska namnet. Saknas koden
// faller vi tillbaka på engelska namnet ur kartdatat.
const N2A = {
  ...
  '270':'GM','275':'PS','276':'DE','288':'GH','292':'GI','300':'GR','304':'GL','308':'GD',
  '312':'GP','316':'GU','320':'GT','324':'GN','328':'GY','332':'HT','336':'VA','340':'HN',
  ...
  '430':'LR','434':'LY','438':'LI','440':'LT','442':'LU','446':'MO','450':'MG','454':'MW',
  '458':'MY','462':'MV','466':'ML','470':'MT','478':'MR','480':'MU','484':'MX','492':'MC',
  '496':'MN','498':'MD','499':'ME','504':'MA','508':'MZ','512':'OM','516':'NA','520':'NR',
  '524':'NP','528':'NL','540':'NC','548':'VU','554':'NZ','558':'NI','562':'NE','566':'NG',
  '570':'NU','578':'NO','580':'MP','583':'FM','584':'MH','585':'PW','586':'PK','591':'PA',
  ...
  '716':'ZW','724':'ES','728':'SS','729':'SD','732':'EH','740':'SR','748':'SZ','752':'SE',
  '756':'CH','760':'SY','762':'TJ','764':'TH','768':'TG','776':'TO','780':'TT','784':'AE',
  '788':'TN','792':'TR','795':'TM','798':'TV','800':'UG','804':'UA','807':'MK','818':'EG',
  '826':'GB','834':'TZ','840':'US','854':'BF','858':'UY','860':'UZ','862':'VE','876':'WF',
  '882':'WS','887':'YE','894':'ZM',
};

let display = null;
try { display = new Intl.DisplayNames(['sv'], { type: 'region' }); } catch {}

export function alpha2(numericId) { return N2A[String(numericId).padStart(3, '0')] || null; }

export function svName(numericId, fallback) {
  const a2 = alpha2(numericId);
  if (a2 && display) {
    try {
      const n = display.of(a2);
      if (n && n !== a2) return n;
    } catch {}
  }
  return fallback || 'Okänt land';
}

// Flaggemoji från alpha-2 (renderas fint på mobiler; på Windows blir det bokstäver)
export function flagEmoji(numericId) {
  const a2 = alpha2(numericId);
  if (!a2) return '';
